...
export default function AnalyticsTab() {
  ...
  const fetchAnalytics = async () => {
    setLoading(true)
    try { const res = await fetch(`/api/admin/analytics?period=${period}`); const result = await res.json(); if (result.success) setData(result) }
  ...
  const periods = [{ value: '7', label: 'Last 7 Days' }, { value: '30', label: 'Last 30 Days' }, { value: '90', label: 'Last 90 Days' }, { value: '365', label: 'Last Year' }]
  ...
  if (!data) return <div className="text-center py-20"><Activity className="w-12 h-12 text-[#64748B] mx-auto mb-4" /><p className="text-[#94A3B8]">No analytics data available</p><p className="text-[#64748B] text-sm mt-2">Run the earnings migration and import data first</p></div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <button onClick={() => setShowPeriodDropdown(!showPeriodDropdown)} className="flex items-center gap-2 px-4 py-2 bg-[#1A1F3A] border border-[#2D3B5F] rounded-lg text-white hover:border-[#0EA5E9] transition-colors"><Calendar className="w-4 h-4 text-[#64748B]" />{periods.find(p => p.value === period)?.label}<ChevronDown className="w-4 h-4 text-[#64748B]" /></button>
            {showPeriodDropdown && <div className="absolute right-0 mt-2 w-48 bg-[#1A1F3A] border border-[#2D3B5F] rounded-lg shadow-lg z-10">{periods.map(p => <button key={p.value} onClick={() => { setPeriod(p.value); setShowPeriodDropdown(false) }} className={`w-full px-4 py-2 text-left text-sm hover:bg-[#2D3B5F] transition-colors ${period === p.value ? 'text-[#0EA5E9]' : 'text-white'}`}>{p.label}</button>)}</div>}
          </div>
          <button onClick={fetchAnalytics} className="p-2 bg-[#1A1F3A] border border-[#2D3B5F] rounded-lg text-[#64748B] hover:text-white hover:border-[#0EA5E9] transition-colors"><RefreshCw className="w-5 h-5" /></button>
        </div>
      </div>

      <div className="bg-gradient-to-br from-[#0EA5E9]/20 to-[#10F981]/20 border border-[#0EA5E9]/30 rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div><p className="text-[#94A3B8] text-sm mb-1">Total Network Earnings</p><p className="text-4xl font-bold text-white">{formatCurrency(summary.totalUsd)}</p><p className="text-[#64748B] text-sm mt-2">{periods.find(p => p.value === period)?.label} • Combined SYX + SYH</p></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1A1F3A] border border-[#2D3B5F] rounded-xl overflow-hidden">
          <div className="p-4 border-b border-[#2D3B5F] bg-purple-500/10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3"><div className="w-10 h-10 bg-purple-500/20 rounded-lg flex items-center justify-center"><span className="text-purple-400 font-bold">SYH</span></div><div><h3 className="text-white font-semibold">Helium Mobile</h3><p className="text-[#64748B] text-xs">30d Avg: ${formatNumber(prices.hnt.avg30d, 2)}</p></div></div>
              <div className="text-right"><p className="text-2xl font-bold text-purple-400">{formatCurrency(summary.helium.usd)}</p><p className="text-[#64748B] text-xs">{formatNumber(summary.helium.tokens, 4)} HNT</p></div>
            </div>
          </div>
          <div className="p-4 space-y-4">
      ...
      <div className="bg-[#1A1F3A] border border-[#2D3B5F] rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-[#0A0F2C] rounded-lg p-4"><p className="text-[#64748B] text-xs mb-1">XNET Current</p><p className="text-white font-medium">${formatNumber(prices.xnet.current || 0, 4)}</p></div>
          <div className="bg-[#0A0F2C] rounded-lg p-4"><p className="text-[#64748B] text-xs mb-1">XNET 30d Avg</p><p className="text-orange-400 font-medium">${formatNumber(prices.xnet.avg30d, 4)}</p></div>
          <div className="bg-[#0A0F2C] rounded-lg p-4"><p className="text-[#64748B] text-xs mb-1">HNT Current</p><p className="text-white font-medium">${formatNumber(prices.hnt.current || 0, 2)}</p></div>
          <div className="bg-[#0A0F2C] rounded-lg p-4"><p className="text-[#64748B] text-xs mb-1">HNT 30d Avg</p><p className="text-purple-400 font-medium">${formatNumber(prices.hnt.avg30d, 2)}</p></div>
        </div>
      </div>
      ...
      {(charts.byVenue.xnet.length > 0 || charts.byVenue.helium.length > 0) && (
        <div className="bg-[#1A1F3A] border border-[#2D3B5F] rounded-xl p-6">
          ...
          <div className="space-y-3">
            {[...charts.byVenue.xnet, ...charts.byVenue.helium].reduce((acc: any[], item) => { const existing = acc.find(a => a.id === item.id); if (existing) { existing.usd += item.usd } else { acc.push({ ...item }) }; return acc }, []).sort((a, b) => b.usd - a.usd).slice(0, 10).map((venue, idx) => (
              <div key={venue.id} className="flex items-center gap-4"><span className="text-[#64748B] w-6">{idx + 1}.</span><span className="text-white flex-1">{venue.name}</span><span className="text-[#0EA5E9] font-medium">{formatCurrency(venue.usd)}</span><div className="w-32 bg-[#2D3B5F] rounded-full h-2"><div className="bg-gradient-to-r from-[#0EA5E9] to-[#10F981] h-2 rounded-full" style={{ width: `${Math.min((venue.usd / (charts.byVenue.xnet[0]?.usd || 1)) * 100, 100)}%` }} /></div></div>
            ))}
          </div>
        </div>
      )}
      ...
      {(charts.byPartner.xnet.length > 0 || charts.byPartner.helium.length > 0) && (
        <div className="bg-[#1A1F3A] border border-[#2D3B5F] rounded-xl p-6">
          ...
          <div className="space-y-3">
            {[...charts.byPartner.xnet, ...charts.byPartner.helium].reduce((acc: any[], item) => { const existing = acc.find(a => a.id === item.id); if (existing) { existing.usd += item.usd } else { acc.push({ ...item }) }; return acc }, []).sort((a, b) => b.usd - a.usd).slice(0, 10).map((partner, idx) => (
              <div key={partner.id} className="flex items-center gap-4"><span className="text-[#64748B] w-6">{idx + 1}.</span><span className="text-white flex-1">{partner.name}</span><span className="text-[#10F981] font-medium">{formatCurrency(partner.usd)}</span><div className="w-32 bg-[#2D3B5F] rounded-full h-2"><div className="bg-gradient-to-r from-[#10F981] to-[#0EA5E9] h-2 rounded-full" style={{ width: `${Math.min((partner.usd / (charts.byPartner.xnet[0]?.usd || 1)) * 100, 100)}%` }} /></div></div>
            ))}
          </div>
        </div>
      )}
      ...
      {summary.totalUsd === 0 && (
        <div className="bg-[#1A1F3A] border border-[#2D3B5F] rounded-xl p-8 text-center">
          ...
          <p className="text-[#94A3B8] text-sm max-w-md mx-auto">Earnings data will appear here once XNET and Helium data is synced from your SharePoint. Make sure to run the database migration and set up Power Automate flows.</p>
        </div>
      )}
    </div>
  )
