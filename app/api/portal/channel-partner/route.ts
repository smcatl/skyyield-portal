// app/api/portal/channel-partner/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Find channel partner by user link or email
    let partner = null

    if (userData?.channel_partner_id) {
      const { data } = await supabase
        .from('channel_partners')
        .select('*')
        .eq('id', userData.channel_partner_id)
        .single()
      partner = data
    }
    ...
    if (!partner) {
      return NextResponse.json({ 
        success: false, 
        error: 'No channel partner account found for this user' 
      }, { status: 404 })
    }

    // Fetch referrals (location partners referred by this channel partner)
    ...
    console.error('Channel partner portal error:', error)
    return NextResponse.json({ 
      success: false, 
